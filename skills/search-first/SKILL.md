...
2. **Need analysis.** Define the required functionality in one or two sentences and the constraints:
   language/framework/runtime (read `./CONTEXT.md`; default to the stack there), license posture,
   and the compliance requirements from `CONTEXT.md` or the working agreement.
3. **Search in parallel.**
   - **Repo first** — is it already solved internally? (`Grep`/`Explore` for existing utilities/patterns.)
   - **Registries** — the stack's package registry (PyPI, npm, NuGet, crates.io, …).
...
4. **Evaluate the candidates.** Score on fitness, maintenance/release cadence, community, docs,
   **license + compliance posture**, and dependency footprint. For more than two candidates,
