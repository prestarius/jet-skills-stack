...
6. **Verification plan** — how the executing agent proves the work is done: the commands that must pass, a one-line `/goal` condition it can set for the session, and which checks belong in a `Stop` hook rather than in prose. Name the human validation zones (auth, payments, prod data) that need explicit sign-off.
7. **Exact commands** — setup/run/test commands, per the stack and the locale defaults in the working agreement.
8. **Open questions** — anything the human must still decide.
